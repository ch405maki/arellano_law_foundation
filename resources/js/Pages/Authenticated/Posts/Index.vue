    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex justify-between items-center">
                ...
                <!-- Create BTN -->
                <div class="relative">
                <a :href="route('posts.create')" class="bg-gray-800 hover:bg-gray-700 text-white px-4 py-[5px] rounded-lg flex items-center gap-2">
                    <i class="fa-solid fa-plus"></i> Post
                </a>
                </div>
            </div>
        </template>
        <div class="flex flex-col bg-gray-100 mb-8">
                                    ...
                                    <table class="min-w-full">
                                        <thead>
                                            <tr class="bg-gray-200">
                                                <th scope="col" class="p-4 text-left text-sm leading-6 font-semibold text-gray-900 capitalize"> Title | Department | Content </th>
                                            </tr>
                                        </thead>
                                        <tbody class="divide-y divide-gray-300">
                                            ...
                                            <tr v-for="post in posts" :key="post.id" class="bg-white transition-all duration-500 hover:bg-gray-50">
                                                <td class="p-4 flex flex-col justify-center">
                                                    <h1 class="text-sm font-medium text-gray-900">{{ post.title }}</h1>
                                                    <h1 class="text-sm font-medium text-gray-600">{{ post.user.department }}</h1>
                                                    <p class="text-sm text-gray-900" v-html="post.content.slice(0, 200) + (post.content.length > 200 ? '...' : '')"></p>
                                                </td>           
                                                <td class="p-2 flex items-center gap-2 justify-end">
                                                    <a :href="route('post.show', { id: post.id })" class="p-2 rounded-full hover:text-red-600 group flex item-center active:ring-2 active:ring-offset-2 active:ring-indigo-600" title="Preview">
                                                    ...
                                                    <!-- Delete Button -->
                                                    <button @click="confirmDelete(post.id)" class="p-2 rounded-full hover:text-red-600 group flex item-center active:ring-2 active:ring-offset-2 active:ring-red-600">
        ...
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div class="hidden sm:block absolute top-0 right-0 pt-4 pr-4">
                <button @click="isModalOpen = false" class="bg-white rounded-md text-gray-400 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
            ...
            <div class="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
                <button @click="deletePost" class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-red-600 text-base font-medium text-white hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 sm:ml-3 sm:w-auto sm:text-sm">
                Delete
                </button>
                <button @click="isModalOpen = false" class="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 sm:mt-0 sm:w-auto sm:text-sm">
                Cancel
                </button>
            </div>
            </div>
...
const deletePost = () => {
  axios
    .delete(route('posts.destroy', postIdToDelete.value))
    .then(() => {
